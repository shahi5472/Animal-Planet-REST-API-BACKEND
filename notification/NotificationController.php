...
class NotificationController extends DB_Functions
{
    public static function create($userId, $data)
    {
        ...
        return self::db()->createNotification($userId, $data, $created_at, $updated_at);
    }
    ...
    public static function createDoctorAdminNotification($data)
    {
        $created_at = date("Y-m-d  h:i:s");
        $updated_at = date("Y-m-d  h:i:s");
        ...
        foreach ($result as $item) {
            self::db()->createNotification($item['id'], $data, $created_at, $updated_at);
        }
        return true;
    }
}